...
class BondFloatRBF(torch.nn.Module):
    ...
    def __init__(self, bond_float_names, embed_dim, rbf_params=None):
        ...
        for name in self.bond_float_names:
            ...
            linear = torch.nn.Linear(len(centers), embed_dim).cpu()
...
class GINNodeEmbedding(torch.nn.Module):
    ...
    def __init__(self, num_layers, emb_dim, drop_ratio=0.5, JK="last", residual=False):
        ...
        for layer in range(num_layers):
            ...
            # self.convs.append(TransformerConv(in_channels=emb_dim, out_channels=emb_dim, edge_dim=emb_dim))
            # self.convs_bond_angle.append(GINConv(emb_dim))
            self.convs_bond_angle.append(GATConv(emb_dim,emb_dim))
            # self.convs_bond_angle.append(TransformerConv(in_channels=emb_dim, out_channels=emb_dim, edge_dim=emb_dim))
    ...
    def forward(self, batched_atom_bond,batched_bond_angle):
        ...
        for layer in range(self.num_layers):
            h = self.convs[layer](x=h_list[layer], edge_index=edge_index, edge_attr=h_list_ba[layer])
...
class GINGraphPooling(nn.Module):
    ...
    def forward(self,atom_bond_data,bond_angle_data):
        # batched_atom_bond = batched_data['atom_bond_data']
        # batched_bond_angle = batched_data['bond_angle_data']
        batched_atom_bond =atom_bond_data
        batched_bond_angle =bond_angle_data
